...
// Read configuration from environment variables (Supports both GREEN_API_INSTANCE and GREEN_API_ID_INSTANCE)
const idInstance = (process.env.GREEN_API_INSTANCE || process.env.GREEN_API_ID_INSTANCE || '').trim();
...
if (!idInstance || !apiTokenInstance || !chatId) {
  ...
  console.error('Please verify the secrets in GitHub Settings ➔ Secrets and variables ➔ Actions:');
  if (!idInstance) console.error('  - GREEN_API_INSTANCE (or GREEN_API_ID_INSTANCE)');
